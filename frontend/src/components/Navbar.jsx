          ...
          <nav className="hidden md:flex items-center gap-1.5 lg:gap-2">
            ...
            {isAuthenticated && isEmployer && (
              <div className="flex items-center gap-2">
                ...
                >
                  <Layers className="w-3.5 h-3.5 text-[#2F80ED] dark:text-blue-400" />
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-[#E5E7EB] dark:border-slate-800 bg-white dark:bg-slate-900 shadow-[0_20px_40px_rgb(0,0,0,0.1)] absolute top-[80px] w-full animate-fadeIn">
          <div className="px-6 pt-6 pb-8 space-y-3">
            ...
            {isAuthenticated && isEmployer && (
              <div className="flex items-center justify-between px-4 py-3 rounded-[14px] hover:bg-[#EDF5FF] dark:hover:bg-slate-800 transition-colors">
                <Link
                  to="/post-job"
                  onClick={closeMenu}
                  className="flex items-center gap-3 text-[15px] font-medium text-[#111827] dark:text-slate-200 hover:text-[#2F80ED] dark:hover:text-blue-400"
                >
                  <PlusCircle className="w-5 h-5 text-[#9CA3AF] dark:text-slate-400" />
                  <span>Post a Job</span>
                </Link>
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#EDF5FF] dark:bg-slate-800 border border-[#2F80ED]/20 text-[#2F80ED] dark:text-blue-400 text-[11px] font-bold">
                  <Layers className="w-3 h-3" />
                  {postedJobsCount} Added
                </span>
              </div>
            )}
